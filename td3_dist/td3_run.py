...
from td3_dist.td3_agent import TD3Agent, td3_torcs, run_worker, queue_worker
import visdom
import numpy as np
import random
import argparse
...
def get_env_params(args, env, goal_set):
    ...
    params = {'obs': obs['observation'].shape[-1],
              ...
              'action_max': env.action_space.high.flatten()[0],
...
def playGame(args_, train=True, episode_count=2000):
    ...
    env_params = get_env_params(args, env, None)
    ...
    eval_queue = mp.Queue(maxsize=100)
    lock, eval_lock = mp.Lock(), mp.Lock()
    remotes, work_remotes = zip(*[Pipe() for _ in range(args_.nenvs)])
    ps = [Process(target=run_worker, args=(index, args_, env_config, env_params, agent.actor_local, agent.actor_eval,
                                           lock, eval_lock, queue, eval_queue)) for index in range(args_.nenvs)]
    ps.append(Process(target=queue_worker, args=(lock, queue, agent.memory)))
    for p in ps:
