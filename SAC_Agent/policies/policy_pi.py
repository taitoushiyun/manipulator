...
class TanhGaussianPolicy(Mlp, ExplorationPolicy):
    def __init__(self, hidden_sizes, obs_dim, action_dim, std=None,
                 init_w=1e-3, **kwargs):

        self.save_init_params(locals())
        super(TanhGaussianPolicy, self).__init__(hidden_sizes, input_size=obs_dim, output_size=action_dim,
                                                 init_w=init_w, **kwargs)

        self.std, self.log_std = std, None
        if std is None:
    ...
    def forward(self, obs, return_log_prob=False,
        ...
        if self.std is None:
            ...
            std, log_std = self.std, self.log_std
