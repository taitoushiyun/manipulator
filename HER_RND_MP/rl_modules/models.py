...
class RND1(nn.Module):
    def __init__(self,  env_params, args):
        super(RND1, self).__init__()
        self.layer = nn.Sequential(nn.Linear(env_params['obs'], 256),
                                   nn.ReLU(),
                                   nn.Linear(256, 256),
                                   nn.ReLU(),
                                   nn.Linear(256, 256))
...
def create_pop_art_cls(lower_layer):
    class PopArtCritic(nn.Module):
        def __init__(self, env_params, args):
            super(PopArtCritic, self).__init__()
            self.lower_layer = lower_layer(env_params, args)
            self.upper_layer = CriticUpperLayer(env_params, args)
