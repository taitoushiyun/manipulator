...
class ddpg_agent:
    def __init__(self, args, env, env_params):
        ...
        if self.args.use_popart:
            ...
            self.critic_explore_optim = torch.optim.Adam(self.critic_explore_network.lower_layer.parameters(),
        ...
        self.result_deque = deque(maxlen=100)
        self.score_deque = deque(maxlen=100)
        self.eval_result_queue = deque(maxlen=100)
        ...
        for epoch in range(self.args.n_epochs):
            for n_cycle in range(self.args.n_cycles):
                ...
                for _ in range(self.args.num_rollouts_per_mpi):
                    ...
                    i_episode += 1
                    self.result_deque.extend(result)
                    ...
                    logger.info("Episode: %d,          Path length: %d       result: %f       reward: %f"
                                % (i_episode, episode_length, sum(result)/len(result), score))
                ...
                mb_obs = batch_process(np.array(mb_obs))
                mb_ag = batch_process(np.array(mb_ag))
                mb_g = batch_process(np.array(mb_g))
                mb_actions = batch_process(np.array(mb_actions))
                # store the episodes
    ...
    def _update_network(self):
        ...
        mu_plus_sigma_explore = self.pop_art_explore.mu + self.pop_art_explore.sigma if self.pop_art_explore is not None else torch.tensor(
            0)
        mu_minis_sigma_explore = self.pop_art_explore.mu - self.pop_art_explore.sigma if self.pop_art_explore is not None else torch.tensor(
            0)
        pop_is_active = torch.tensor(self.pop_art.pop_is_active) if self.pop_art is not None else torch.tensor(0)
        pop_is_active_explore = torch.tensor(
            self.pop_art_explore.pop_is_active) if self.pop_art_explore is not None else torch.tensor(0)
        debug_items = dict()
        for debug_item, value in self.debug_items.items():
            if value.get('legend') is None:
    ...
    def _eval_agent(self, n_epoch):
        ...
        total_success_rate = batch_process(np.array(total_success_rate))
